...
export default function FlowList({
  ...
  return (
    <aside
      className={`${
        isVisible ? "w-[280px]" : "w-20"
      } h-[calc(100vh-20px)] mt-2.5 ml-1 rounded-lg bg-white p-3 border-r border-gray-200 flex flex-col gap-2 fixed z-10 text-sm shadow-lg transition-all duration-300`}
    >
      <div className="font-semibold text-[20px] mb-3 mt-1 text-black/90 uppercase border-b border-black/10 pb-1.5 flex items-center justify-center gap-2 flex-nowrap overflow-hidden">
        <img
          src={icon}
          alt="Flow Icon"
          ...
          title="Flow List"
        />
        <p className={`${isVisible ? "visible" : "hidden"} text-nowrap`}>
          Flow Analyzer
        </p>
      </div>
        ...
        {isLoading ? (
          ...
          >
            {flows?.length ? (
              ...
              <p className="text-nowrap">No data.</p>
            )}
          </div>
        )}
      </div>
